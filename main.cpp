        ...
        for(int i = 0; i < hero.mage_info().spellbook_size(); i++) {
            switch(hero.mage_info().spellbook(i)) {
                case Spell::FROSTBOLT:
                    cout << "frostbolt, ";
                    break;
                case Spell::FIRESELFDESTRUCT:
                    cout << "fireselfdestruct, ";
                    break;
                default:
                    cout << "(unknown spell), ";
                    break;
...
int main() {
    ...
    Hero warrior;
    warrior.set_name("Juno");
    warrior.set_hp(150);
    warrior.set_xp(1000);
    warrior.mutable_warrior_info()->set_weapon(Weapon::SWORD);
    warrior.mutable_warrior_info()->set_arrows_number(81);

    Hero mage;
    mage.set_name("Davis");
    mage.set_hp(145);
    mage.set_xp(1024);
    mage.mutable_mage_info()->add_spellbook(Spell::FROSTBOLT);
    mage.mutable_mage_info()->add_spellbook(Spell::FIRESELFDESTRUCT);
    mage.mutable_mage_info()->set_mana(100);

    cout << "Saving heroes..." << endl;
    saveHero("juno.dat", warrior);
    saveHero("davis.dat", mage);

    cout << "Loading heroes..." << endl;
    Hero warrior2;
    Hero mage2;
    loadHero("juno.dat", warrior2);
    loadHero("davis.dat", mage2);
